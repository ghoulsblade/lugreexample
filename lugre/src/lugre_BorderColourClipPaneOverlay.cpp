...
cBorderColourClipPaneOverlay::cBorderColourClipPaneOverlay(const Ogre::String& name) 
	: cColourClipPaneOverlay(name), mpBorderMaterial(), mBorderRenderable(0), mpRobRenderOpBorder(&mRenderOpBorder) {
	mBorder.left = 0;
	mBorder.top = 0;
	mBorder.right = 0;
	mBorder.bottom = 0;
...
void cBorderColourClipPaneOverlay::updatePositionGeometry(void) {
	...
	*/
	...
	if (mbClipInitialized) {
		...
		Lugre::Rectangle clippingRegion = mClip;
		if (mMetricsMode != GMM_RELATIVE) {
...
struct VertexRect {
	...
	VertexRect	Intersect	(const Lugre::Rectangle2D& clippingRegion);
};
#endif
