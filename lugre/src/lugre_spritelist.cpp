...
inline void	RectAdd	(Lugre::Rectangle& a,const float l,const float t,const float r,const float b) {
...
inline void	RectAdd	(Lugre::Rectangle& a,const Lugre::Rectangle& b) { RectAdd(a,b.left,b.top,b.right,b.bottom); }
inline void	RectAddWithOffset	(Lugre::Rectangle& a,const Lugre::Rectangle& b,const Ogre::Vector3& off) { 
	RectAdd(a,off.x+b.left,off.y+b.top,off.x+b.right,off.y+b.bottom);
}
inline void	RectSetWithOffset	(Lugre::Rectangle& a,const Lugre::Rectangle& b,const Ogre::Vector3& off) { 
	a.left		= off.x+b.left;
	a.top		= off.y+b.top;
...
inline Lugre::Rectangle	MakeRectLTRB	(const float left,const float top,const float right,const float bottom) {
	Lugre::Rectangle r;
	r.left		= left;
	r.top		= top;
	r.right		= right;
...
inline Lugre::Rectangle	OffsetRect	(const Lugre::Rectangle& rIn,const Ogre::Vector3& vAdd) {
...
void	cRenderGroup2D::CalcAbsBounds				(Lugre::Rectangle& r) {
	Lugre::Rectangle& a = GetRelBounds();
...
void	cRenderGroup2D::UpdateClip	() {
	...
	if (_ParentClipActive()) {
		...
		mrTmpClip = mbClipActive ? OffsetRect(mrClip,GetDerivedPos()).intersect(mpParent->mrTmpClip) : mpParent->mrTmpClip;
...
void	cSpriteList::UpdateGeometryClipped	(const Lugre::Rectangle& rClip) {
	int c = mlSprites.size();
	mbGeometryDirty = false;
...
bool	cSpriteList::cSprite::WriteGeometryClipped	(cRobRenderOp& pGeometry,const bool bVertexCol,const Lugre::Rectangle& rClip) {
